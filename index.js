...
async function procesarUsuario(usuario) {
  ...
  console.log(
    `Procesando usuario -> ${usuario.nombre} ${usuario.apellido1} ${usuario.apellido2}`
  );
  ...
  try {
    ...
    for (let enlaceDia of enlaces) {
      ...
      let {
        ...
        selBtnInscripcion,
        isDisponible,
      } = await checkDisponibilidad(usuario, page, enlaceDia);

      // Si está disponible y ha podido realizar la inscripcion
      if (
        isDisponible &&
        (await realizarInscripcion(usuario, page, selBtnInscripcion, dia, hora))
      ) {
    ...
    const textoWA = `${emoji.get(
      "robot_face"
    )} Error! ocurrido a las ${dateTime.getDateTimeNow()} -> ${error.message}`;
    clienteWA.CrearMensajePOST(textoWA, usuario.movilNotifE164);
  }
}
...
async function checkDisponibilidad(usuario, page, enlace) {
  ...
  if (inscripcion.isDisponible)
    console.log(
      `OK. Hay plaza libre encontrada en el día ${inscripcion.dia} a la hora ${inscripcion.hora}`
    );
  else
    console.log(
      `KO. No hay plazas libres en el día ${inscripcion.dia} a la hora ${inscripcion.hora}`
    );
...
async function realizarInscripcion(
  ...
  let enlaces = await page.evaluate(() =>
    Array.from(document.querySelectorAll(".contenido ul li a")).map(
      (el) => el.href
    )
  );
  ...
  try {
    ...
    let textoError =
      "KO! No ha podido completar la inscripción-> " + error.message;
    console.log(textoError);

    // Notificamos por WhatsApp el error
    const textoWA = `${emoji.get(
      "robot_face"
    )} Error! ocurrido a las ${dateTime.getDateTimeNow()}: ${textoError}`;
    clienteWA.CrearMensajePOST(textoWA, usuario.movilNotifE164);
  }
